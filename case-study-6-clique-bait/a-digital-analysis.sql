...
-- pct = number of visits that view the checkout page but do not have a purchase event /
-- number of visits that view the checkout page
-- Method 1
...
AS       (SELECT E.VisitId
               ...
               , EI.EventName
               , E.SequenceNumber
               , CASE WHEN EI.EventName = N'Add to Cart' THEN 1 WHEN EI.EventName = N'Purchase' THEN 0 END AS ItemsInCart
          FROM   cliquebait.Events AS E
                 INNER JOIN cliquebait.PageHierarchy AS PH
                     ON E.PageId = PH.PageId
                 INNER JOIN cliquebait.EventIdentifier AS EI
...
SELECT   PH.PageName
       , SUM(ItemsInCart) AS ProductsPurchase
